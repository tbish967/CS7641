...
for kernel in kernels:
  ...
  for count in sample_counts:
    print(f'Running for {count} samples')
  ...
  sps[0].plot(sample_counts, np.array(train_scores)*100, label=f'{kernel} kernel')
  sps[1].plot(sample_counts, np.array(test_scores)*100)


sps[0].set_title(f'SVM - {dataset} - Training Set Size')
sps[0].set_ylabel('Training Set Accuracy (%)')
sps[0].legend()
sps[1].set_ylabel('Test Set Accuracy (%)')
sps[1].set_xlabel('Training Samples')
plt.savefig(f'{root}/images/svm/{dataset}_Samples.png')
plt.show()